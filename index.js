...
const getHueColorConfig = function(input, setColor, config) {
...
const file_path = require('os').homedir() + '/.hue/active/colors';
const input = fs.existsSync(file_path) ? fs.readFileSync(file_path) : null;

exports.decorateConfig = config => {
    if (input) {
        return getHueColorConfig(input, setColor, config);
    } else {
        console.log('Couldn\'t find hue cached colors. Run hue first.');
        return Object.assign({}, config, {});
    }
};
